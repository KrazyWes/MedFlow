"""
Step 07a — DOH k-means metrics.

Loads labels + feature matrix per config, computes silhouette / Calinski-Harabasz / Davies-Bouldin,
writes per-dataset text summaries, bar+table PNGs, and a cross-dataset spider chart when enough
datasets succeed.

Inputs: DOH 04_clustering *_kmeans.csv + 02_data_transformation *_features_minmax.csv
Outputs: webp/EDA_and_visualization/DOH/07_evaluation/kmeans/
"""
...
def _plot_kmeans_spider(results: list[tuple[str, float]], out_dir: str) -> None:
    ...
    # Qualitative palette: continuous colormaps made some datasets look like the same line.
    ...
    for i, (cfg_name, sil, ch, db) in enumerate(full_results):
        ...
        # Very light fill so overlapping regions do not turn into muddy brown.
    ...
    plt.tight_layout(rect=[0.02, 0.06, 0.62, 0.88])
...
def main() -> None:
    ...
    out_dir = os.path.join(webp_root_doh(), "07_evaluation", "kmeans")
    _ensure_dir(out_dir)
    term_log = os.path.join(logs_dir_doh(), "07_evaluation_kmeans_doh_terminal.txt")
