"""
Step 04a — DOH k-means.

For each DOH lens config, loads the MinMax feature matrix, searches k in a bounded range,
scores candidates with silhouette (subsampled when n is huge), fits the winning k, and writes
clustering_DOH_*_kmeans.csv (all original columns + cluster_kmeans).

Diagnostics (silhouette-vs-k, elbow-style notes, params txt) land under
webp/EDA_and_visualization/DOH/04_clustering_implementation/kmeans/.
"""
...
def main() -> None:
    ...
    out_dir = os.path.join(webp_root_doh(), "04_clustering_implementation", "kmeans")
    _ensure_dir(cluster_dir)
    _ensure_dir(out_dir)
    term_log = os.path.join(logs_dir_doh(), "04_clustering_implementation_kmeans_doh_terminal.txt")
    with tee_stdio_to_file(term_log):
        _run_kmeans_doh(cluster_dir, out_dir)


def _run_kmeans_doh(cluster_dir: str, out_dir: str) -> None:
    ...
    if sweep_by_dataset:
        ...
        cfg_names = [c.name for c in get_doh_dataset_configs() if c.name in sweep_by_dataset]
        ...
        fig.suptitle("K-Means Hyperparameter Tuning (Silhouette + n_clusters) — DOH (A–E)", fontsize=14, fontweight="bold")
