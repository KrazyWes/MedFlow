"""
Step 06b — DOH DBSCAN figures.

PCA plots with noise points styled distinctly from dense clusters; includes whole-dataset and
per-cluster panels where it makes sense.

Inputs: this_datasets/DOH/04_clustering/*_dbscan.csv
Outputs: webp/EDA_and_visualization/DOH/06_visualization/dbscan/
"""
...
def main() -> None:
    ...
    out_dir = os.path.join(webp_root_doh(), "06_visualization", "dbscan")
    _ensure_dir(out_dir)
    term_log = os.path.join(logs_dir_doh(), "06_visualization_dbscan_doh_terminal.txt")
