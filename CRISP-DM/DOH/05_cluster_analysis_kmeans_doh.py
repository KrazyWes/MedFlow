"""
Step 05a — DOH k-means cluster profiles.

Turns labeled CSVs into tabular summaries: how big each cluster is, mean/median feature patterns,
and per-cluster slice exports for offline inspection.

Reads this_datasets/DOH/04_clustering/*_kmeans.csv; writes CSVs + any companion text under
webp/EDA_and_visualization/DOH/05_cluster_analysis/kmeans/.
"""
...
from _common import _ensure_dir, get_doh_dataset_configs
...
def main() -> None:
    ...
    out_dir = os.path.join(webp_root_doh(), "05_cluster_analysis", "kmeans")
    _ensure_dir(out_dir)
    term_log = os.path.join(logs_dir_doh(), "05_cluster_analysis_kmeans_doh_terminal.txt")
