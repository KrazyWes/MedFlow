"""
Step 05b — DOH DBSCAN cluster profiles.

Summarizes dense clusters plus the noise bucket (-1): sizes, feature means, optional noise-only
exports for qualitative review.

Inputs: this_datasets/DOH/04_clustering/*_dbscan.csv
Outputs: webp/EDA_and_visualization/DOH/05_cluster_analysis/dbscan/
"""
...
def main() -> None:
    ...
    out_dir = os.path.join(webp_root_doh(), "05_cluster_analysis", "dbscan")
    _ensure_dir(out_dir)
    term_log = os.path.join(logs_dir_doh(), "05_cluster_analysis_dbscan_doh_terminal.txt")
