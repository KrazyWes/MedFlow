"""
Step 04b — PhilGEPS DBSCAN.

Same algorithm as DOH DBSCAN with extra guards for very large n (subsampled ε sweeps, trimmed
multi-panel figures). Expect long wall-clock on lens B even after optimizations.

Outputs: this_datasets/PhilGEPS/04_clustering/*_dbscan.csv + diagnostics under
webp/EDA_and_visualization/PhilGEPS/04_clustering_implementation/dbscan/.
"""
...
from _common import _ensure_dir, _get_data_source_colors, data_source_for_dataset_name, get_philgeps_dataset_configs, load_features
...
def main() -> None:
    ...
    out_dir = os.path.join(webp_root_philgeps(), "04_clustering_implementation", "dbscan")
    _ensure_dir(cluster_dir)
    _ensure_dir(out_dir)
    term_log = os.path.join(logs_dir_philgeps(), "04_clustering_implementation_dbscan_philgeps_terminal.txt")
    with tee_stdio_to_file(term_log):
        _run_dbscan_philgeps(cluster_dir, out_dir)


def _run_dbscan_philgeps(cluster_dir: str, out_dir: str) -> None:
    for cfg in get_philgeps_dataset_configs():
        ...
        # --- Step 3: Eps sensitivity (subsample very large n so sweep finishes; final fit uses full X) ---
        sweep_cap = 12000
        ...
        if use_sweep_subsample:
            # Multi-panel replays extra full-data DBSCAN at other eps values; skip for very large n (too slow).
            if n_samples > 20000:
